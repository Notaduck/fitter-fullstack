...
func (s *PostgresStore) createRecordMsgTable() error {

	// delete the old table
	// query := `DROP TABLE IF EXISTS record_msgs`

	// s.db.Exec(query)

	query := `
	CREATE TABLE IF NOT EXISTS record_msgs (

    id                            SERIAL PRIMARY KEY,
	activity_id				      INTEGER NOT NULL,
    accumulated_power             BIGINT,
    ...
    compressed_accumulated_power  BIGINT,
    compressed_speed_distance     BYTEA,
    cycle_length                  SMALLINT,
    cycles                        SMALLINT,
    device_index                  SMALLINT,
    distance                      INTEGER,
    enhanced_altitude             BIGINT,
    enhanced_speed                BIGINT,
	...
	query = `CREATE INDEX IF NOT EXISTS idx_activity_id on record_msgs(activity_id); `
	_, err = s.db.Exec(query)
